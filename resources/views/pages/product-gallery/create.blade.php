        ...
        <div class="card-body card-block">
            <form action="{{ route('product-gallery.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="file" name="photo" value="{{ old('photo') }}" accept="image/*" required class="form-control-file @error('photo') is-invalid @enderror"/>
                    @error('photo') <div class="text-muted">{{ $message }}</div>@enderror
                </div>
                <div class="form-group">
                    <label for="is_default" class="form-control-label">Jadikan Default</label><br>
                    <label>
                        <input type="radio" name="is_default" value="1" required class="form-control-input @error('is_default') is-invalid @enderror">&nbsp; Ya
                    </label>
                    &nbsp;
                    <label>
                        <input type="radio" name="is_default" value="0" required class="form-control-input @error('is_default') is-invalid @enderror">&nbsp; Tidak
                    </label>
